...
# function to find distance in 3D space
def distance(vertex0, vertex1) :

    # variables hold distance between coordinates on each axis
    x_distance = vertex0.coords[0] - vertex1.coords[0]
    y_distance = vertex0.coords[1] - vertex1.coords[1]
    z_distance = vertex0.coords[2] - vertex1.coords[2]

    # using pythagorean theorem to determine the distance between the verticies in 3D space
    dist = math.sqrt(pow(x_distance, 2) + pow(y_distance, 2) + pow(z_distance, 2))
...
def buildTriangles( slice0, slice1 ):
    ...
    # variables hold the number of vertices of a slice
    length_slice0 = len(slice0.verts)
    length_slice1 = len(slice1.verts)
    # setting min_distance to an absurdly large number
    min_distance = 10000000000008
    ...
    #
    for i in range(length_slice0) :
        for j in range(length_slice1) :

            # call distance function with vertex from each slice
            new_distance = distance(slice0.verts[i], slice1.verts[j])
            # check if the distance is less than the minimum distance
            if (new_distance < min_distance) :
                # sets min distance to the new distance
                min_distance = new_distance
                # sets the index equal to index of those vertices
                index0 = i
                index1 = j
    ...
    # for loop to run through all vertices of slice0 accounting for the wrap around cases
    for i in range (index0, length_slice0 + index0) :
        new_slice0.append(slice0.verts[i % length_slice0])

    # for loop to run through all vertices of slice1 accounting for the wrap around cases
    for i in range (index1, length_slice1 + index1) :
        new_slice1.append(slice1.verts[i % length_slice1])

    # append starting vert
    new_slice0.append(slice0.verts[index0])
    new_slice1.append(slice1.verts[index1])
    ...
    #
    minArea = [[0 for i in range(len(new_slice0))] for j in range(len(new_slice1))]      # CHANGE THIS
    minDir  = [[None for i in range(len(new_slice0))] for j in range(len(new_slice1))]      # CHANGE THIS
    ...
    # initializing previous to 0
    previous = 0
    # for loop used to fill in the row of the case that there isn't a row-1
    for i in range(1, len(new_slice0)) :
        minArea[0][i] = triangleArea(new_slice1[0].coords, new_slice0[i-1].coords, new_slice0[i].coords) + previous
        previous = minArea[0][i]
        minDir[0][i] = Dir.PREV_COL

    # Fill in col 0 of minArea and minDir, since it's a special case as there's no col -1
    #
    # [2 marks]
    # resetting previous to 0
    previous = 0
    # for loop used to fill in the col of the case that there isn't a col-1
    for i in range(1, len(new_slice1)) :
        minArea[i][0] = triangleArea(new_slice0[0].coords, new_slice1[i-1].coords, new_slice1[i].coords) + previous
        previous = minArea[i][0]
    ...
    # for loop used to fill in the table with areas and record the optimal direction to minDir
    for i in range(1, len(new_slice1)) :
        for j in range(1, len(new_slice0)) :
            triangle_up = triangleArea(new_slice1[i].coords, new_slice1[i-1].coords, new_slice0[j].coords)
            triangle_left = triangleArea(new_slice0[j].coords, new_slice0[j-1].coords, new_slice1[i].coords)
            # starting from bottom right corner of the table and working out
            # if the calculated area of the triangle one row up is smaller
            if ( (triangle_up + minArea[i-1][j])  <  (triangle_left + minArea[i][j-1]) ) :
                # add into the table
                minArea[i][j] = (triangle_up + minArea[i-1][j])
                minDir[i][j] = Dir.PREV_ROW
            # if the calculated area of the triangle one col to the left is smaller
            else :
                # add into table
                minArea[i][j] = (triangle_left + minArea[i][j-1])
    ...
    # array to be filled with triangles that are arrays of three verts
    triangles = []

    # table has one less row/col than array new_slice1/new_slice0
    i = len(new_slice1) - 1
    j = len(new_slice0) - 1

    # while we have not traversed the whole table
    while (i != 0 or j != 0) :
        # initialize an empty array for a temporary triangle
        temp = []
        # if the minimum direction at the location in the table is equal to the previous row
        # append corresponding vertices to the temporary triangle
        if minDir[i][j] == Dir.PREV_ROW :
            temp.append(new_slice1[i])
            temp.append(new_slice1[i-1])
            temp.append(new_slice0[j])
            i -= 1
        else :
        # if the minimum direction at the location in the table is equal to the previous column
        # append corresponding vertices to the temporary triangle
            temp.append(new_slice0[j])
            temp.append(new_slice0[j-1])
            temp.append(new_slice1[i])
            j -= 1

        # append the temporary triangle to array triangles
        triangles.append(Triangle(temp))

    # reset the temporary triangle array to be empty
    temp = []

    # edge case if we have not yet completely traversed table through the while loop
    # g0t to the edge of a col/row and not all the way up/left
    # if the minimum direction at the location in the table is equal to the previous row
    # append corresponding vertices to the temporary triangle
    if minDir[i][j] == Dir.PREV_ROW :
        temp.append(new_slice1[i])
